...
public class OrderController {
  ...
  @GetMapping("")
  public ResponseEntity<Page<OrderGeneralResponse>> getOrdersByUserId(PageParamRequest pageParamRequest, @RequestParam("lastStatus") OrderStatus lastStatus) {
    Page<OrderGeneralResponse> orders = orderService.getOrdersByUserId(AuthenticationService.getCurrentUser(), lastStatus, pageParamRequest);
    return ResponseEntity.ok(orders);
  }

  @PostMapping("")
  public ResponseEntity<OrderDetailResponse> createOrderByUserId(@RequestBody OrderCreateRequest request) {
    OrderDetailResponse order = orderService.createOrderByUserId(AuthenticationService.getCurrentUser(), request);
  ...
  public ResponseEntity<OrderDetailResponse> getOrderByUserId(@PathVariable UUID orderId) {
    OrderDetailResponse order = orderService.getOrderDetailResponseByOrderId(AuthenticationService.getCurrentUser(), orderId);
    return ResponseEntity.ok(order);
  }

  @DeleteMapping("/{orderId}")
  public ResponseEntity<Object> deleteOrderByUserId(@PathVariable UUID orderId) {
    orderService.deleteOrderByUserId(AuthenticationService.getCurrentUser(), orderId);
    return ResponseEntity.noContent().build();
  }

  @PostMapping("/{orderId}/orderTransportations")
  public ResponseEntity<Object> addOrderTransportationByUserId(@PathVariable UUID orderId, @RequestBody OrderStatus orderStatus) {
    orderTransportationService.createOrderTransportationByUserId(AuthenticationService.getCurrentUser(), orderId, orderStatus);
    return ResponseEntity.created(null).build();
  }

  @PatchMapping("/{orderId}/orderItems/{orderItemId}")
  public ResponseEntity<OrderItemGeneralResponse> patchOrderByUserId(@PathVariable UUID orderId, @PathVariable UUID orderItemId, @RequestBody OrderItemPatchRequest orderItemPatchRequest) {
    OrderItemGeneralResponse orderItem = orderItemService.patchOrderItemByUserId(AuthenticationService.getCurrentUser(), orderId, orderItemId, orderItemPatchRequest);
  ...
  public ResponseEntity<OrderItemGeneralResponse> createOrderItemByUserId(@PathVariable UUID orderId, @RequestBody OrderItemCreateRequest orderItemCreateRequest) {
    OrderItemGeneralResponse orderItem = orderItemService.createOrderItemByUserId(AuthenticationService.getCurrentUser(), orderId, orderItemCreateRequest);
